...
namespace Chess.Figures
{
    class Quinn : Figure
    {
        public Quinn(int figurecolor, char figurechar, int posx, int posy)
        {
        ...
        {
            if (Math.Abs(EnterNewPosX - posx) == Math.Abs(EnterNewPosY - posy)||Math.Abs(EnterNewPosX - posx)==0|| Math.Abs(EnterNewPosY - posy)==0)
            {
                bool wayfree = false;
                if (EnterNewPosX - posx > 0 && EnterNewPosY - posy > 0)
                {
                    ...
                    {
                        if (Field[EnterNewPosX + i, EnterNewPosY + i].IsCellEmpty(Field, EnterNewPosX, EnterNewPosY))
                        {
                            wayfree = true;
                            continue;
                        }
            ...
            {
                Console.WriteLine("Quinn can`t move this way");
            }
        }
    }
}